...
function applyCubeIsosurface(vol){
  ...
  // 每相位独立阈值：正/负瓣幅值常不对称，用全局 max 会让较弱相位阈值偏高→更碎片化
  const data = vol.data;
  ...
  const posMesh = build(1, isoPos, 0xff7a3d, isoOpacity);       // 正相位：暖橙
  const negMesh = build(-1, isoNeg, 0x3d7aff, isoOpacity * 0.9); // 负相位：冷蓝
