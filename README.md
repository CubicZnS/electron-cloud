...
- **Quantum Data（电子密度 Cube 导入）**：底部 Quantum Data 入口导入 Gaussian cubegen / Multiwfn 导出的单标量场电子密度 `.cube/.cub`（64 MB / 400 万体素上限，本地解析不上传）；bohr→Å 换算并按网格中心居中；粒子按真实密度权重采样（对数密度加权；低密度截断 = max(95% 总质量阈值, 0.1%×峰值) 双保险，松包围盒/非零背景也不会弥散）；复用现有粒子过渡 / 色图 LUT / Bloom / 画质切换；图例标注 `Imported electron density`；导入模式下 Total/Inductive/Resonance 与官能团替换禁用并说明原因，可一键退出回半定量模式；另支持 VASP CHGCAR/CHG 电荷密度（自旋极化取总密度块，无元素行时骨架灰球）
...
- **外部 DFT 验证闭环**：一键导出当前结构为 VASP POSCAR / INCAR / KPOINTS 输入起点（POTCAR 需按 POSCAR 物种顺序自行拼接；K 点与 ISMEAR 按体系核对）；外部跑完后把 CHGCAR/CHG 经 Quantum Data 入口导入查看 DFT 计算的真实电子密度。
- **参考态与组分扫描**：弛豫结果可存为参考，后续同超胞结果显示 ΔE（eV 与 eV/atom，晶胞弛豫时附 ΔV）——MACE 近似，非形成能；对 A/B/C 子晶格整位替换一串元素并串行弛豫，给出 E/ΔE/体积/步数进度表与 CSV 导出。弛豫 0 步收敛且应力显著时，提示对称驻点风险并建议微扰重试。
