...
## [v2.8.0] — 2026-09-05 · 新增：DFT 验证闭环（VASP 导出 / CHGCAR 回导）、参考态比较、组分扫描

- Crystal Lab 新增 VASP 输入导出（POSCAR / INCAR / KPOINTS，POTCAR 按物种顺序自行拼接的提示），把当前可掺杂/空位/超胞结构作为外部 DFT 验证的起点；外部跑完后把 VASP CHGCAR/CHG 经 Quantum Data 入口导入，查看 DFT 计算的真实电子密度（本工具不做密度计算）。
- Quantum Data 现支持 VASP CHGCAR/CHG 电荷密度：POSCAR 头 + 网格体素（x 最快），与 Cube 共用居中/采样/色图/截断管线；自旋极化文件只读取总密度块；无元素符号行时骨架按灰球并诚实标注。字段识别置信「电子密度（高）」（注释含英文 total charge density）。
- 参考态比较：弛豫结果可一键「存为参考」，后续同超胞弛豫在结果面板给出 ΔE（eV 与 eV/atom），晶胞弛豫时附 ΔV；文案明示 MACE 近似、非形成能；原子数不同时仅按原子归一并提示。
- 组分扫描：对 A/B/C 任一子晶格整位替换为逗号分隔的元素列表，引擎串行逐个弛豫，实时进度表（化学式/E 终/ΔE/体积/步数/最大力/状态）与 CSV 导出；任一步后可停止。
- 0 步收敛驻点提示：弛豫返回 0 步且内部应力显著（>2 GPa）时，结果面板黄色警示并建议先施加微扰再重试，避免把对称驻点当平衡。
- 组分扫描结果表改为容器内横向滚动（min-width + overflow-x），不再顶穿 300px 侧栏右边界；参考态描述行在窄屏自动折行。
- 进入/退出晶体模式加入平滑过渡（进入 300ms 淡入 + 底部上移缩放，退出 160ms 淡出后再隐藏），尊重 prefers-reduced-motion。
- 依赖加载器改进：three.js 附加组件（OrbitControls 等）改为逐文件跨 CDN 回退，任一缺失明确报错并提示网络，不再静默缺失导致启动崩溃。
- validate-cube 新增 CHGCAR 解析用例（73 → 80），validate-crystal 新增 toPOSCAR/formula 导出回归（21 → 22）；sigma 15/15、构建语法检查通过。
